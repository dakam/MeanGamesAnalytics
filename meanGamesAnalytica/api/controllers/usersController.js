...
module.exports.Register = function(req, res) {
    console.log("we are here, registration")
    ...
    if(req.body && req.body.username && req.body.password) {
        ...
        res.status(400).json({"message": "Please supply all fields to continue"})
    }
...
module.exports.Login = function(req, res) {
    ...
    if(req.body && req.body.username && req.body.password) {
        ...
        User.findOne({username: req.body.username}).exec(function(err, user) {
            ...
            if (err) {
                console.log("There is an issue in Login");
                response.status = 500;
                response.message = err;
            } 
            else if(!user) {
            ...
            if(user) {
                ...
                bcrypt.compare(req.body.password, user.password,function(err, same) {
                    ...
                    if(same) {
                      ...
                      let token =  jwt.sign({name:user.name}, process.env.SECRET, {expiresIn: 3600})
        ...
        res.status(400).json({"message": "Please supply username and password to continue"})
...
module.exports.authenticate = function (req, res, next) {
    ...
    if (userHeaser) {
        ...
        jwt.verify(token, process.env.SECRET, function(err, decodedToken) {
            if(err) {
                console.log("Encountered error in JWT ", err);
